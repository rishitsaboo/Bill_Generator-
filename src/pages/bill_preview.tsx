...
function BillGenerator() {
  ...
  if (!billRef.current) {
    console.log("❌ Bill ref is NULL");
    return;
  }

  const node = billRef.current;
  
  // Clone the node to avoid affecting the visible element
  ...
  // Hide input field if present
  const inputs = clone.querySelectorAll('input');
  inputs.forEach(input => input.style.display = 'none');
  
  // Set clone position and visibility for capture
  ...
  try {
    const options = {
      quality: 1,
      width: node.offsetWidth * 2, // Higher resolution
    ...
    link.download = `Kavitas-Kitchen-Bill-${new Date().toISOString().split('T')[0]}.png`;
    link.href = dataUrl;
    link.click();
  ...
  return (
    ...
    {/* Customer Name Input ABOVE Bill Preview */}
    ...
    <div className="flex justify-center items-center gap-3 py-4">
        <img src="/mainlogo.png" className="w-20" />
        <div className="text-4xl font-extrabold text-center">
        <p className="tracking-widest" style={{ color: "#5e2c13" }}>
            Kavita's
        </p>
        <p className="tracking-widest" style={{ color: "#edb046" }}>
            Kitchen
        </p>
        </div>
    </div>
    ...
    {/* Items Header */}
    <div className="grid grid-cols-3 font-semibold text-sm pb-2">
        <div>Item</div>
    ...
    {billItems.map((item) => (
        <div key={item.id} className="grid grid-cols-3 text-sm mb-2 pb-1">
        <div>{item.name}</div>
    ...
    <div className="flex justify-between items-start mb-4">
        ...
        <img src="/QRcode.jpg" className="w-28 h-28 object-cover" />
    </div>
    ...
    className="p-2 bg-blue-500 text-white rounded mt-4 hover:bg-blue-600"
    >
    Download Bill
    </button>
